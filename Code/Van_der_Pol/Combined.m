...
for i =1:n_lambda
    ...
    NT_C(i) = norm(v_C - s2(length(s)+1:length(s2)))/n_predictions;
    NT_Q(i) = norm(v_Q - s2(length(s)+1:length(s2)))/n_predictions;
    NT_A(i) = norm(v_A - s2(length(s)+1:length(s2)))/n_predictions;
end
...
title("Prediction error on test data with respect to \lambda with (\mu = " + mu + ")");
